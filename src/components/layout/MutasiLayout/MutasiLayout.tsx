...
function MutasiLayout() {
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      <div id="table-section" className="w-full flex flex-col">
        <div className="flex justify-between bg-primary-dark-blue p-[18px] rounded-t">
        ...
        <div className="bg-neutral-1 p-9 flex flex-col gap-[18px] rounded-b mb-[20px] shadow-box">
          <p
            aria-label="Kriteria Pencarian"
            className="text-primary-dark-blue font-semibold text-[20px]"
          >
            Kriteria Pencarian
          </p>
            ...
            <div className="flex flex-col gap-[18px] w-full">
              ...
              <div className="flex flex-col gap-[18px] items-end">
                <span
                  ...
                  className="font-medium text-sm text-primary-dark-blue"
                >
                  Maksimum rentang Tanggal Awal dan Akhir mutasi adalah 31 hari
                  dan harus masuk dalam periode 6 bulan transaksi terakhir.
                </span>
                <button
                  onClick={handleSearch}
                  className="w-[179px] justify-center flex gap-4 bg-primary-blue px-[36.5px] py-[10px] rounded-xl text-white font-semibold"
                >
                  Cari
                </button>
              </div>
          ...
          {showDatePicker && (
            <DatePicker
              labelPopup="Popup Pilih tanggal kalender"
          ...
          {showDatePicker1 && (
            <DatePicker
              labelPopup="Popup Pilih tanggal kalender"
              handleClose={handleCloseCalendar1}
              handleSubmit={handleSubmitCalendar1}
            />
          )}
        </div>
        {filteredData?.length !== 0 ? (
          <table
            id="table"
            className="border border-primary-dark-blue border-collapse table-auto bg-neutral-1 w-full rounded-t rounded-b shadow-box"
          >
            <thead className="bg-primary-dark-blue text-neutral-1">
              <tr className="h-11">
                <th className="first:rounded-ss-[10px] py-2 border border-primary-dark-blue border-collapse text-base font-semibold">
                  Tanggal
                </th>
                <th className="border border-primary-dark-blue border-collapse text-base font-semibold">
                  Nama
                </th>
                <th className="border border-primary-dark-blue border-collapse text-base font-semibold">
                  No Rekening
                ...
                <th className="rounded-se-[10px] py-2 border border-primary-dark-blue border-collapse text-base font-semibold">
                  Keterangan
                </th>
              </tr>
            </thead>
            <tbody className="text-primary-blue font-semibold text-base">
              {filteredData?.map((data) => (
                <tr className="text-center h-11" key={data.transactionId}>
                  <td className="py-1 border border-primary-dark-blue border-collapse p-[10px]">
                    {formatDateTable(data.transactionDate)}
                  </td>
                  <td className="border border-primary-dark-blue border-collapse p-[10px]">
                    {data.beneficiaryAccount.beneficiaryAccountName}
                  </td>
                  <td className="border border-primary-dark-blue border-collapse p-[10px]">
                    {data.beneficiaryAccount.beneficiaryAccountNumber}
                  </td>
                  <td className="border border-primary-dark-blue border-collapse p-[10px]">
                    Rp. {formatCurrency(data.amount.value)}
                  </td>
                  <td className="border border-primary-dark-blue border-collapse text-left p-[10px]">
                    {data.desc}
                  </td>
                </tr>
              ))}
          ...
          <div className="flex justify-center bg-neutral-1 shadow-box rounded p-4 text-neutral-9 font-semibold text-md gap-[24px]">
      ...
      <div className="flex justify-between">
        <div>
          <div className="flex flex-row bg-neutral-1 rounded shadow-box p-4 text-neutran-1 text-base w-[286px] gap-[24px]">
            {monthlyBankStatement ? (
              <>
                ...
                <div className="flex flex-col w-full font-normal">
                  <span aria-label="Jumlah Saldo Awal">
                    : Rp.{" "}
                    {formatCurrency(
                      bankStatement?.accountBalance?.startingBalance?.value
                    ) ?? "N/A"}
                  </span>
                  <span aria-label="Jumlah Mutasi Kredit">
                    : Rp.{" "}
                    {formatCurrency(monthlyBankStatement?.monthlyIncome?.value)}
                  </span>
                  <span aria-label="Jumlah Mutasi Debit">
                    : Rp.{" "}
                    {formatCurrency(
                      monthlyBankStatement?.monthlyOutcome?.value
                    )}
                  </span>
                  <span aria-label="Jumlah Saldo Akhir">
                    : Rp.{" "}
                    {formatCurrency(
                      bankStatement?.accountBalance?.endingBalance?.value
                    ) ?? "N/A"}
                  </span>
                </div>
              </>
